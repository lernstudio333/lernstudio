...
describe('rawCardToApiCard — one test per card type', () => {
  ...
  it('SINGLE_CARD: question left of marker, answer right', () => {
    const raw = {
      cardId:  '1000000000001',
      type:    'SINGLE_CARD',
      question: 'The powerhouse of the cell is',
      answer:  'the mitochondria',
      parents: ['Biology', 'Cell Biology'],
      flags:   [],
      rowIndex: 4,
  ...
  it('SINGLE_CARD: answer is undefined when nothing is right of the marker', () => {
    const raw = {
      cardId: '1000000000002', type: 'SINGLE_CARD',
      question: 'Define osmosis', answer: undefined,
      parents: ['Biology'], flags: [], rowIndex: 7,
    };
    const result = rawCardToApiCard(raw, DOC_ID);
  ...
  it('GAP: question contains [...], answer is the gap word', () => {
    const raw = {
  ...
  it('MULTI_CARD: question is the heading, answer is array of bullet children', () => {
    const raw = {
      cardId:  '1000000000004',
      type:    'MULTI_CARD',
      question: 'Biosphere',
      answer:  ['liquid body', 'living field'],
      parents: ['Craniosacral Therapy', 'Foundations'],
      flags:   [],
      rowIndex: 28,
  ...
  it('MULTI_CARD: answer is undefined when there are no children', () => {
    const raw = {
      cardId: '1000000000005', type: 'MULTI_CARD',
  ...
  it('SYNONYM: question is the term, answer is array of synonyms', () => {
    const raw = {
    ...
    expect(result.question).toBe('Synonyme');
    expect(result.answer).toEqual(['Craniosacral-Rhythmus', 'Primäre Respiration', 'CRI']);
  });

  it('IMAGES: question is the label, answer is array of image references', () => {
    const raw = {
  ...
  it('parents are not included in the question for any type', () => {
    const raw = {
      cardId: '1000000000008', type: 'SINGLE_CARD',
      question: 'What is ATP?', answer: 'Adenosine triphosphate',
      parents: ['Biology', 'Biochemistry', 'Energy'], flags: [], rowIndex: 9,
    };
    const result = rawCardToApiCard(raw, DOC_ID);
    expect(result.question).toBe('What is ATP?');
    expect(JSON.stringify(result)).not.toContain('Biology');
  });
